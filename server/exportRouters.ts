...
export const exportRouter = router({
  ...
  /**
   * Export custom report: Accreditations by Supplier
   */
  exportAccreditationsBySupplier: protectedProcedure
    .input(z.object({
      eventId: z.number().optional(),
    }))
    .mutation(async ({ input, ctx }) => {
      ...
      for (const event of events) {
        ...
        for (const supplier of suppliers) {
          const supplierAccreditations = await Promise.all(
            accreditations.map(async (acc) => {
              const collab = await db.getCollaboratorById(acc.collaboratorId);
              return collab?.supplierId === supplier.id ? acc : null;
            })
          ).then(results => results.filter(Boolean) as any[]);
          
          if (supplierAccreditations.length === 0) continue;
          
          const pending = supplierAccreditations.filter(acc => acc.status === 'pendente').length;
          const filled = supplierAccreditations.filter(acc => acc.status !== 'pendente').length;
          ...
          reportData.push({
            ...
            pending,
            filled,
            total: supplierAccreditations.length,
          });
        }
      }
      ...
      const ExcelJS = (await import('exceljs')).default;
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('Status por Fornecedor');

      worksheet.columns = [
        { header: 'Fornecedor', key: 'supplierName', width: 30 },
        { header: 'Evento', key: 'eventName', width: 30 },
        { header: 'Pendentes', key: 'pending', width: 15 },
        { header: 'Preenchidos', key: 'filled', width: 15 },
      ...
      const buffer = await workbook.xlsx.writeBuffer();
      const fileName = `relatorio-status-fornecedor-${Date.now()}.xlsx`;
      const fileKey = `exports/${ctx.user.id}/${fileName}`;
      const { url } = await storagePut(fileKey, buffer as any, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

      await db.createExport({
        eventId: input.eventId,
        exportType: 'report_accreditations_by_supplier',
        fileName,
        fileUrl: url,
        fileKey,
        filters: JSON.stringify(input),
  ...
  exportAccreditations: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Enrich with collaborator, supplier and job function data
      const enrichedData = await Promise.all(
        accreditations.map(async (acc) => {
          const collaborator = await db.getCollaboratorById(acc.collaboratorId);
          const supplier = collaborator ? await db.getSupplierById(collaborator.supplierId) : null;
