...
const eventFormSchema = z.object({
  ...
  eventDate: z.string().min(1, "Data do evento é obrigatória").optional().or(z.literal("")),
  registrationDeadline: z.string().min(1, "Prazo de cadastro é obrigatório").optional().or(z.literal("")),
  credentialReleaseDate: z.string().min(1, "Data de liberação é obrigatória").optional().or(z.literal("")),
...
export function EventForm({ open, onOpenChange, event }: EventFormProps) {
  ...
  useEffect(() => {
    if (!event && eventDate) {
      ...
      // Default registration deadline: D-4
      const deadline = new Date(date);
      deadline.setDate(date.getDate() - 4);
      form.setValue("registrationDeadline", deadline.toISOString().split('T')[0]);

      // Default credential release: D-3
      const release = new Date(date);
      release.setDate(date.getDate() - 3);
      form.setValue("credentialReleaseDate", release.toISOString().split('T')[0]);
    }
  }, [eventDate, event, form]);
