      ...
      <div className="space-y-6">
              ...
              <SelectContent>
                {events?.filter(e => e.status === "aberto").map(event => (
                  <SelectItem key={event.id} value={event.id.toString()}>
                    {event.name} - {new Date(event.eventDate).toLocaleDateString("pt-BR")}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        {selectedEventId && (
          <>
                      ...
                      return (
                        <div 
                          ...
                          className={`flex items-center gap-4 p-3 border rounded-lg ${
                            !canAdd ? "opacity-50 bg-muted" : "hover:bg-accent cursor-pointer"
                          }`}
                          onClick={() => {
                            if (!canAdd) return;
            ...
            <Card>
              <CardHeader>
                <CardTitle>Colaboradores Credenciados</CardTitle>
                <CardDescription>
                  {accreditations?.length || 0} colaboradores credenciados neste evento
                </CardDescription>
              </CardHeader>
              <CardContent>
